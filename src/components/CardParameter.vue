...
<template>
  ...
  >
    <div class="text-base mb-3">{{ props.cardItem?.title }}</div>
    <div
    ...
    >
      ...
      >
        <div class="text-xs mb-2 capitalize">{{ formatTitle(item?.subtitle) }}</div>
        <div class="text-base">{{ calculateValue(item?.value) }}</div>
      </div>
    </div>
  </div>
</template>
